/**
 * Board helpers for both ways of playing: against the computer and against
 * another student.
 *
 * A verbatim port of the web app's `lib/chess-core.ts` — same chess.js, same
 * needs — so the two boards cannot drift apart in what they consider legal.
 * The backend grades moves with its own engine regardless; this copy exists so
 * the board can highlight squares without a round trip.
 */
